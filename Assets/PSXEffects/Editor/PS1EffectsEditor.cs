...
public class PSXEffectsEditor : Editor {
	...
	SerializedProperty clampAffine;
	SerializedProperty affineBounds;
	...
	void OnEnable() {
		...
		clampAffine = serializedObject.FindProperty("clampAffine");
		affineBounds = serializedObject.FindProperty("affineBounds");
	...
	public override void OnInspectorGUI() {
		...
		if (psfx.sections[1]) {
			...
			clampAffine.boolValue = EditorGUILayout.Toggle("Clamp Affine Mapping", clampAffine.boolValue);
			if (clampAffine.boolValue) {
				affineBounds.floatValue = EditorGUILayout.Slider("Clamp Bounds", affineBounds.floatValue, 0, 100);
			}
